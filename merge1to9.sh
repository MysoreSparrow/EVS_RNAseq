...
do
	...
	cat 22030a00${i}_01_S${i}_L00[1,2]_R1_001.fastq.gz > 22030a00${i}_01_S${i}_lm_R1.fastq.gz
	# merge for L1R2 and L2R2 and obtain lanemerged (lm) R2 file
	cat 22030a00${i}_01_S${i}_L00[1,2]_R2_001.fastq.gz > 22030a00${i}_01_S${i}_lm_R2.fastq.gz

	# For 02
	# merge for L1R1 and L2R1 and obtain lanemerged (lm) R1 file
	cat 22030a00${i}_02_S${i}_L00[1,2]_R1_001.fastq.gz > 22030a00${i}_02_S${i}_lm_R1.fastq.gz
	# merge for L1R2 and L2R2 and obtain lanemerged (lm) R2 file
	cat 22030a00${i}_02_S${i}_L00[1,2]_R2_001.fastq.gz > 22030a00${i}_02_S${i}_lm_R2.fastq.gz
	
	# Now perform Sample merging of the lane merged files
	# merge 01_lm_R1 and 02_lm_R1 into single samplemerged(sm) R1 file
	cat 22030a00${i}_0[1,2]_S${i}_lm_R1.fastq.gz > AJ${i}_sm_R1.fastq.gz
	# merge 01_lm_R2 and 02_lm_R2 into single samplemerged(sm) R2 file
	cat 22030a00${i}_0[1,2]_S${i}_lm_R2.fastq.gz > AJ${i}_sm_R2.fastq.gz
	echo Completed $i
done
...
for i in {1..9}
do
	fastp -i AJ${i}_sm_R1.fastq.gz -I AJ${i}_sm_R2.fastq.gz -o out.${i}_sm_R1.fastq.gz -O out.${i}_sm_R2.fastq.gz --length_required 20 --detect_adapter_for_pe --correction --low_complexity_filter --trim_poly_g --trim_poly_x --overrepresentation_analysis --thread 8
done
